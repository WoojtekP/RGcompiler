...
void keeperCompletion(game_state &state, const uint depth) {
  while (state.get_current_player() == keeper) {
    state.get_all_moves(moves[depth]);
    if (moves[depth].size() != 1) exitError("Keeper has " + std::to_string(moves[depth].size()) + " moves");
    state.apply_move(moves[depth][0]);
  }
}

void doPerft(game_state &state, const uint depth) {
  numStates++;
  ...
  for (uint i = 0; i < moves[depth].size(); i++) {
    ...
    nextState.apply_move(moves[depth][i]);
