...
ulong sumScores[reasoner::NUM_PLAYERS], minScores[reasoner::NUM_PLAYERS], maxScores[reasoner::NUM_PLAYERS];
...
void doSimulation() {
  ...
  while (true) {
    ...
    if constexpr(!BENCHMARK) {
      if (moves.size() == 0) exitWithError(state, "Player " + std::to_string(state.getSymbolName(state.getCurrentPlayer())) + " has 0 moves");
    }
  ...
  if constexpr(!BENCHMARK) {
    ...
    for (uint player = 0; player < reasoner::NUM_PLAYERS; player++) {
      uint score = state.getPlayerScore(state.getPlayerSymbol(player));
